    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      ...
      <div class="m-2 p-4 bg-slate-100 rounded">
        <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-2">
          <h2 class="text-4xl font-bold dark:text-white">Edit Menu</h2>
          <form method="POST" action="{{ route('admin.menus.update', $menu->id) }}" enctype="multipart/form-data">
            ...
            <div class="sm:col-span-6 mb-2">
              ...
              <div class="mt-1">
                <input type="text" id="name" name="name" class="block w-full appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5" value="{{ $menu->name }}" autofocus />
              </div>
              @error('name')
            ...
            <div class="sm:col-span-6 mb-2">
              ...
              <div class="mt-1">
                <input type="number" id="price" name="price" min="0.00" max="10000.00" step="0.01" class="block w-full appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5" value="{{ $menu->price }}" />
              </div>
              @error('price')
                <div class="text-small text-red-400">{{ $message }}</div>
              @enderror
            ...
            <div class="sm:col-span-6 mb-2">
              ...
              <div class="mt-1">
                <textarea id="description" name="description" rows="3" class="shadow-sm focus:ring-indigo-500 appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out focus:border-indigo-500 block w-full sm:text-sm">{{ $menu->description }}</textarea>
              </div>
              @error('description')
                <div class="text-small text-red-400">{{ $message }}</div>
              @enderror
